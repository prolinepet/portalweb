...
export default function PriceTableListPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const q = query.trim();
      const url = q ? `/api/base/price-tables?q=${encodeURIComponent(q)}` : "/api/base/price-tables";
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return rows;
    return rows.filter((r) => (r.nrtabpre || "").toLowerCase().includes(q) || (r.descricao || "").toLowerCase().includes(q));
  }, [rows, query]);

  const onDelete = async (id: number) => {
    if (!confirm("Excluir esta tabela de preço?")) return;
    try {
      const res = await fetch(`/api/base/price-tables/${id}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || `Erro ${res.status}`);
      await load();
    } catch (e: any) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => router.push("/base/price-tables/maintenance")} className="px-3 py-2 rounded bg-green-600 text-white text-sm">
          Novo
        </button>
      </div>
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          placeholder="Buscar por Cód Tab ou Descrição"
            ...
            {loading ? (
              ...
              filtered.map((r) => (
                <tr key={r.id} className="border-t">
                  <td className="px-3 py-2">{r.nrtabpre}</td>
                  <td className="px-3 py-2">{r.descricao}</td>
                  <td className="px-3 py-2">{situacaoLabel(r.situacao)}</td>
                  <td className="px-3 py-2 text-right">
                    <div className="inline-flex items-center gap-2">
                      <button onClick={() => router.push(`/base/price-tables/maintenance?id=${r.id}`)} className="px-2 py-1 rounded border text-sm">
                        Editar
                      </button>

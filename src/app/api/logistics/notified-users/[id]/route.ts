...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const updated = await prisma.logisticPhaseNotifiedUser.update({
      where: { id: Math.trunc(id) },
      data: { userId: Math.trunc(userId), allowReturn, allowNext, permissions },
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.logisticPhaseNotifiedUser.delete({ where: { id: Math.trunc(id) } });

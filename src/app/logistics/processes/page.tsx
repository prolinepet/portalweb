...
type LogisticProcess = { id: number; code: number; description: string; isActive: boolean };

export default function LogisticProcessesPage() {
  const router = useRouter();
  const [rows, setRows] = useState<LogisticProcess[]>([]);
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/logistics/processes", { cache: "no-store" });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <h1 className="text-xl font-semibold">Logística • Processos Logísticos</h1>
        <div className="ml-auto flex items-center gap-2">
          <button
            type="button"
            onClick={() => router.push("/logistics/processes/new")}
                  ...
                  <tr key={p.id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-center">
                      <div className="inline-flex">
                        <button
                          type="button"
                          onClick={() => router.push(`/logistics/processes/${p.id}`)}

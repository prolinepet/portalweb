...
function pathToProgramCodes(pathname: string): string[] {
  ...
  if (pathname.startsWith("/logistics/panel")) return ["PAINEL_LOGISTICO"];
...
export default function ProgramGuard({ perms, children, pathname }: { perms: Permissions | null; children: React.ReactNode; pathname: string | null }) {
  const codes = pathname ? pathToProgramCodes(pathname) : [];
  if (!codes.length || !perms || !perms.activeEntityId) return <>{children}</>;
  const allowed = perms.modules.some((m) => m.programs.some((p) => codes.includes(p.code)));
